...
export function Column({
  ...
  return (
    <div className="flex flex-col rounded-lg bg-card border" aria-label={`${title} column`}>
      ...
      <div ref={listRef} data-status={status} className="p-2 space-y-2 min-h-[120px]" aria-live="polite">
        {ideas.map((it) => (
          <Card key={it.id} data-id={it.id} className={statusClass[status] + " cursor-grab active:cursor-grabbing"}>
