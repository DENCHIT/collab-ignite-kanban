-- SECURITY FIXES: RLS Policies and Secure Functions
...
-- 5. Remove plaintext passcode column (CRITICAL)
ALTER TABLE public.boards_secrets DROP COLUMN IF EXISTS passcode_plain;

-- 6. Update to strong bcrypt hashing
...
DECLARE 
  passcode_hash TEXT;
BEGIN
  -- Generate bcrypt hash (cost 12 for strong security)
...
-- 7. Recreate secure functions (no passcode exposure)
