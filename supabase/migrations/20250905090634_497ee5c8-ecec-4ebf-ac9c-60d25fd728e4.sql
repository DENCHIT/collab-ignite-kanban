...
-- 1. Drop functions that need to be recreated with different signatures
...
-- 2. DROP overly permissive RLS policies and replace with secure ones
...
-- 3. Create secure RLS policies for ideas
...
-- 4. Create secure RLS policies for boards
...
-- 5. Create secure RLS policy for notifications
...
-- 6. Remove plaintext passcode column (CRITICAL SECURITY FIX)
ALTER TABLE public.boards_secrets DROP COLUMN IF EXISTS passcode_plain;

-- 7. Update passcode functions to use strong bcrypt hashing
...
-- 8. Recreate SECURITY DEFINER functions with proper security
...
-- 9. Secure add_board_member function with proper authorization
