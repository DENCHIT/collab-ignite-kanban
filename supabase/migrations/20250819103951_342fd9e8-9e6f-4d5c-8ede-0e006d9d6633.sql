...
AS $$
  ...
  BEGIN
    ...
    -- Verify passcode using simple hash comparison (SHA256)
    RETURN stored_hash = encode(digest(_passcode, 'sha256'), 'hex');
  END;
$$;
...
AS $$
  DECLARE 
    passcode_hash TEXT;
  BEGIN
    -- Generate hash for passcode using SHA256
    passcode_hash := encode(digest(_passcode, 'sha256'), 'hex');
