...
AS $$
  DECLARE 
    ...
    computed_hash TEXT;
  BEGIN
    -- Check if board exists and get hash + salt
    SELECT bs.passcode_hash, bs.salt INTO stored_hash, stored_salt
    ...
    -- Compute hash with salt using MD5 (available by default)
    computed_hash := md5(stored_salt || _passcode || stored_salt);
...
AS $$
  ...
  BEGIN
    ...
    INSERT INTO public.boards_secrets (board_id, passcode_hash, salt)
    VALUES (_board_id, passcode_hash, salt_value)
